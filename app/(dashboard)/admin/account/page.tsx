...
function InfoRow({ label, value }: { label: string; value?: string | null }) {
  return (
    <div className="flex flex-col gap-0.5">
      ...
      <dd className="text-sm text-gray-800 dark:text-gray-100">{value ?? <span className="text-gray-300">-</span>}</dd>
    </div>
  );
}
...
function Card({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
      <h2 className="text-sm font-semibold text-gray-700 mb-5 pb-3 border-b border-gray-100 dark:border-gray-700">{title}</h2>
      {children}
    </div>
  );
}
...
export default async function AdminAccountPage() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      {/* Avatar + identity */}
      <div className="flex items-center gap-4 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Administrator · Pronuvia</p>
          <span className="inline-flex items-center gap-1 mt-2 px-2.5 py-1 bg-gray-900/10 text-[#3DBFA4] rounded-full text-xs font-semibold">
