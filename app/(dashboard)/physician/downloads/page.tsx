...
export default async function PhysicianDownloadsPage() {
  ...
  return (
    <div className="max-w-3xl">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-gray-800">Downloads</h1>
        <p className="text-sm text-gray-500 mt-0.5">Educational and clinical materials provided by Pronuvia.</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        {docs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">Materials will appear here when uploaded by your administrator.</p>
          </div>
        ) : (
            ...
            {docs.map((doc) => (
              <li key={doc.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50/50 transition-colors group">
                ...
                <a href={doc.fileUrl} target="_blank" rel="noopener noreferrer"
                  className="shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#5BB8D4]/10 text-[#5BB8D4] text-xs font-medium rounded-lg hover:bg-[#5BB8D4]/20 transition-colors">
