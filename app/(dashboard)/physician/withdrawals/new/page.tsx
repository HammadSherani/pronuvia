...
export default async function PhysicianNewWithdrawPage() {
  ...
  return (
    <div className="max-w-lg space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/physician/withdrawals" className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-600 transition-colors">
      ...
      {hasPending && (
        <div className="bg-blue-50 border border-blue-200 rounded-2xl px-5 py-5">
          ...
          <Link href="/physician/withdrawals" className="mt-3 inline-flex text-sm text-blue-700 font-medium hover:underline">
            View status →
          </Link>
        </div>
      )}

      {physician?.bankName && !hasPending && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
          ...
          <div className="bg-gray-50 rounded-xl px-4 py-3.5 space-y-1">
            ...
            <p className="text-sm font-semibold text-gray-800">{physician.bankAccountName}</p>
            <p className="text-xs text-gray-500">{physician.bankName}</p>
            {physician.bankAccountNumber && (
              <p className="text-xs font-mono text-gray-600">
                ••••{physician.bankAccountNumber.slice(-4)}
              </p>
            )}
          </div>
