...
export function NewWithdrawForm({ maxAmount }: { maxAmount: number }) {
  const [state, action, pending] = useActionState(createPhysicianWithdrawRequest, undefined);
    ...
    <form action={action} className="space-y-4">
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            type="number" name="amount" step="0.01" min="10" max={maxAmount}
            placeholder="0.00"
            className="w-full pl-8 pr-3.5 py-2.5 text-sm border border-gray-200 bg-gray-50 rounded-xl outline-none focus:border-gray-900 focus:bg-white transition-colors"
          />
        </div>
      </div>
