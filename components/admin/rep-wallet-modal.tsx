...
export function RepWalletModal({
  salesRepId,
  repName,
}: {
  salesRepId: string;
  ...
  useEffect(() => {
    if (!open) return;
    setLoading(true);
    getRepWalletDetails(salesRepId).then((d) => {
      setData(d);
      setLoading(false);
    });
  }, [open, salesRepId]);
  ...
  const balance = data?.rep?.walletBalance ?? 0;
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-[#5BB8D4]/10 text-[#5BB8D4] border border-[#5BB8D4]/30 rounded-lg hover:bg-[#5BB8D4]/20 transition-colors"
      >
                    ...
                    <div className="bg-gray-50 border border-gray-100 rounded-xl p-4">
                      ...
                      {data.rep.bankName ? (
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-1.5">
                            <svg className="w-3.5 h-3.5 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M3 6l9-3 9 3M3 6v12l9 3 9-3V6" />
                            </svg>
                            <p className="text-xs text-gray-600">{data.rep.bankName}</p>
                          </div>
